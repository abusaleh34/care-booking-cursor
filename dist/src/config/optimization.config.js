...
exports.default = (0, config_1.registerAs)('optimization', () => ({
    performance: {
        compression: {
    ...
    cache: {
        ...
        ttl: {
            searchResults: parseInt(process.env.CACHE_TTL_SEARCH || '900'),
            providerProfiles: parseInt(process.env.CACHE_TTL_PROVIDER || '3600'),
            serviceCategories: parseInt(process.env.CACHE_TTL_CATEGORIES || '86400'),
            userSessions: parseInt(process.env.CACHE_TTL_SESSION || '3600'),
            availabilityData: parseInt(process.env.CACHE_TTL_AVAILABILITY || '300'),
            dashboardMetrics: parseInt(process.env.CACHE_TTL_DASHBOARD || '300'),
            staticContent: parseInt(process.env.CACHE_TTL_STATIC || '604800'),
        },
        prefixes: {
            search: 'search:',
            provider: 'provider:',
    ...
    security: {
        ...
        brute: {
            ...
            minWait: parseInt(process.env.BRUTE_MIN_WAIT || '5000'),
            maxWait: parseInt(process.env.BRUTE_MAX_WAIT || '900000'),
            lifetime: parseInt(process.env.BRUTE_LIFETIME || '3600000'),
        },
    },
    storage: {
    ...
    thresholds: {
        apiResponseTime: {
            p50: parseInt(process.env.THRESHOLD_API_P50 || '100'),
            p95: parseInt(process.env.THRESHOLD_API_P95 || '200'),
            p99: parseInt(process.env.THRESHOLD_API_P99 || '500'),
        },
        databaseQuery: {
            slow: parseInt(process.env.THRESHOLD_DB_SLOW || '1000'),
            critical: parseInt(process.env.THRESHOLD_DB_CRITICAL || '5000'),
        },
        cacheHitRate: {
            warning: parseFloat(process.env.THRESHOLD_CACHE_WARNING || '0.7'),
            critical: parseFloat(process.env.THRESHOLD_CACHE_CRITICAL || '0.5'),
        },
        errorRate: {
            warning: parseFloat(process.env.THRESHOLD_ERROR_WARNING || '0.01'),
            critical: parseFloat(process.env.THRESHOLD_ERROR_CRITICAL || '0.05'),
        },
    },
}));

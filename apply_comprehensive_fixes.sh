...
if [ -f "tests/unit/customer/search-filters.spec.ts" ]; then
  ...
  # Update the beforeEach to properly set up all dependencies
  sed -i.bak '/const module: TestingModule = await Test.createTestingModule({/,/}).compile();/c\
...
\
    ...
    }).compile();' "tests/unit/customer/search-filters.spec.ts"
...
' "tests/unit/customer/search-filters.spec.ts"
      rm "tests/unit/customer/search-filters.spec.ts.bak2"
    fi
...
if [ -f "tests/unit/customer/search.service.spec.ts" ]; then
  ...
  sed -i.bak '/const module: TestingModule = await Test.createTestingModule({/,/}).compile();/c\
    ...
    }).compile();' "tests/unit/customer/search.service.spec.ts"
...
' "tests/unit/customer/search.service.spec.ts"
      rm "tests/unit/customer/search.service.spec.ts.bak2"
    fi
...
- Fixed cache service and repository injection in search service tests
